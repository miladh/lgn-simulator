...
cube driftingGrating(double C, double wd, double phase,
                     vec2 kd, vec r, vec t){
    ...
    for(int l = 0; l < int(t.n_elem); l++){
        for(int i = 0; i < int(r.n_elem); i++){
            for(int j = 0; j < int(r.n_elem); j++){
                S(i,j,l) = C * cos(dot(kd, vec2{r[i], r[j]}) - wd * t[l] + phase);
            }
        }
    }
...
cx_cube driftingGratingFourierTransform(double C, double wd, double phase,
                                        vec2 kd, vec k, vec w){
...
void runStimulusFullFieldGratingTest(int ns, int nt, double dt, double ds,
    ...
    cube Sdg = driftingGrating(C, wd, grating.phase(),  grating.kVec(), r, t);
    cx_cube Sdg_ft = driftingGratingFourierTransform(C, wd, grating.phase(), grating.kVec(), k, w);

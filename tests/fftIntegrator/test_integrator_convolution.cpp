...
void runSpatiotemporalDeltaTest(int nt, double dt, int ns, double ds,
    ...
    Gaussian Ws(A, a);
    TemporalDelta Wt(tau);

    SpatialDelta Ks(weight, shift);
    TemporalDelta Kt(delay);
    ...
    for(int l=0; l < int(k.n_elem); l++){
        for(int i = 0; i < int(r.n_elem); i++){
            for(int j = 0; j < int(r.n_elem); j++){
                F_e(i,j,l) = weight * Ws.spatial(vec2{r[i], r[j]}-shift)
                            * Wt.temporal(t[l]- delay)/integrator.temporalResolution();
...
SUITE(integrator){
    TEST(spatiotemporalDelta_test_0) {
        runSpatiotemporalDeltaTest(7, 0.05, 7, 0.05,
                                   0.0, 0.0,

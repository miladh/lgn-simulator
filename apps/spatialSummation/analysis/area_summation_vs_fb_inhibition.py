...
for i in range(1,len(weights),7):
    plot(d,G[i,:], label=r"$\frac{w_{ic}}{w_{rc}}=$"+'{0:.2f}'.format(weights[i]))
legend()
title("Ganglion")
xlabel(r"Spot diameter [deg]", fontsize= 16)
...
figure()
for i in range(1,len(weights),7):
    plot(d,I[i,:], label=r"$\frac{w_{ic}}{w_{rc}}=$"+'{0:.2f}'.format(weights[i]))
legend()
title("Interneuron")
xlabel(r"Spot diameter [deg]", fontsize= 16)
ylabel("response",fontsize= 16)
...
figure()
for i in range(1,len(weights),7):
    plot(d, R[i,:], label=r"$\frac{w_{ic}}{w_{rc}}=$"+'{0:.2f}'.format(weights[i]))
legend()
title("Relay")
xlabel(r"Spot diameter [deg]", fontsize= 16)
ylabel("response",fontsize= 16)

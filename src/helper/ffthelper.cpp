...
mat FFTHelper::fftShift(mat x)
{
    ...
    mat shifted = join_vert(x.rows(n2, n-1), x.rows(0,n2-1));
    shifted = join_horiz(shifted.cols(m2, m-1), shifted.cols(0,m2-1));
...
cube FFTHelper::fftShift(cube x)
{
    cube shifted = x;
    int n = x.n_rows;
    int n2 = int((n+1)/2);
...
cube FFTHelper::ifftShift(cube x)
{
    cube shifted = x;
    int n = x.n_rows;
    int n2 = n-int((n+1)/2);
